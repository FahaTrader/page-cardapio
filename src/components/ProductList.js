...
import './ProductList.css'; // Adicione uma folha de estilos para aplicar CSS
import io from 'socket.io-client'; // Para comunicação em tempo real

const initialCart = [];
const categories = ['Hamburguer', 'Pizza', 'Bebidas'];
const socket = io('http://192.168.0.229:5000'); // Endereço do seu backend
...
const ProductList = ({ route }) => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('http://192.168.0.229:5000/api/products');
  ...
  const handleCallWaiter = async () => {
    try {
      await axios.post('http://192.168.0.229:5000/api/call-waiter', { tableNumber });
      socket.emit('callWaiter', { tableNumber }); // Emite um evento para o backend
      ...
      <div className="product-list">
        {products.map((item) => (
          <div key={item.id} className="product-container">
            {item.image ? (
              <img src={`http://192.168.0.229:5000${item.image}`} alt={item.name} className="product-image" />
            ) : (
